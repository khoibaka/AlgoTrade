...
from src.data.io import Save, Load
...
class BinanceDataFetch:
    ...
    def fetch_kline_server(self, symbol, interval, start_time, end_time=None, limit=10000, data_type='json', save_dir=None):
        ...
        while (len(kline_data) > 0
            ...
            start_ms = kline_data[-1][0] + TimeConverter.interval_to_ms(interval)
        ...
        if save_dir:
            name = Save.get_file_name(
                symbol=symbol,
                start_time=TimeConverter.ms_to_datetime(real_start_time),
                end_time=TimeConverter.ms_to_datetime(real_end_time),
                interval=interval
            )
            print(name)
...
if __name__ == '__main__':
    ...
    bnf.fetch_kline_server('BTCUSDT', '1h', '08-17-2017', '08-17-2020', None, 'csv', 'F:\Projects\AlgoTradeStorage\CSV')
